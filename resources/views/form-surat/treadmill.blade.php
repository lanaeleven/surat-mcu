...
<div class="flex-row bg-white p-5 rounded-lg my-6">
    ...
    <form action="{{ $action }}" method="post" @if (isset($blank)) target="_blank" @endif>
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div>
                <x-date-input name="tanggalPemeriksaan" id="tanggalPemeriksaan" value="{{ old('tanggalPemeriksaan', $treadmill->tanggalPemeriksaan ?? '') }}" :required="true" :readonly="$readonly">Tanggal Pemeriksaan</x-date-input>
            </div>
            <div>
            ...
            <div class="mt-6 flex items-center justify-start gap-x-6">
                @if ($readonly)
                <x-yellow-link-button href="{{ route('treadmill.edit', ['treadmill' => $treadmill->id]) }}">Edit Data</x-yellow-link-button>
