...
<div class="flex-row bg-white p-5 rounded-lg my-6">
    ...
    <form action="{{ $action }}" method="post" @if (isset($blank)) target="_blank" @endif>
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-4 mb-6">
            ...
            <div>
                <x-text-input name="noSurat" id="noSurat" value="{{ old('noSurat', $kesehatanBadan->noSurat ?? '') }}" :required="false" :readonly="$readonly">Nomor Surat</x-text-input>
            </div>
            <div>
        ...
        <div class="grid grid-cols-4 gap-x-32 gap-y-4 my-6">
            <div>
                <x-text-input name="tinggiBadan" id="tinggiBadan" value="{{ old('tinggiBadan', $kesehatanBadan->tinggiBadan ?? '') }}" :required="false" :readonly="$readonly">Tinggi Badan (cm)</x-text-input>
            </div>
            <div>
                <x-text-input name="denyutNadi" id="denyutNadi" value="{{ old('denyutNadi', $kesehatanBadan->denyutNadi ?? '') }}" :required="false" :readonly="$readonly">Denyut Nadi (x/menit)</x-text-input>
            </div>
            <div>
                <x-text-input name="tekananDarah" id="tekananDarah" value="{{ old('tekananDarah', $kesehatanBadan->tekananDarah ?? '') }}" :required="false" :readonly="$readonly">Tekanan Darah (mmHg)</x-text-input>
            </div>
            <div>
                <x-text-input name="spo2" id="spo2" value="{{ old('spo2', $kesehatanBadan->spo2 ?? '') }}" :required="false" :readonly="$readonly">SPO2 (%)</x-text-input>
            </div>
            <div>
                <x-text-input name="beratBadan" id="beratBadan" value="{{ old('beratBadan', $kesehatanBadan->beratBadan ?? '') }}" :required="false" :readonly="$readonly">Berat Badan (Kg)</x-text-input>
            </div>
            <div>
                <x-text-input name="frekuensiNafas" id="frekuensiNafas" value="{{ old('frekuensiNafas', $kesehatanBadan->frekuensiNafas ?? '') }}" :required="false" :readonly="$readonly">Frekuensi Nafas (x/menit)</x-text-input>
            </div>
            <div>
                <x-text-input name="suhuBadan" id="suhuBadan" value="{{ old('suhuBadan', $kesehatanBadan->suhuBadan ?? '') }}" :required="false" :readonly="$readonly">Suhu Badan (<span>&deg;</span>C) </x-text-input>
            </div>
            <div>
                <x-text-input name="imt" id="imt" value="{{ old('imt', $kesehatanBadan->imt ?? '') }}" :required="false" :readonly="$readonly">IMT (kg/m<sup>2</sup>)</x-text-input>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-4 mt-6">
            ...
            <div>
                <x-text-input name="keperluanSurat" id="keperluanSurat" value="{{ old('keperluanSurat', $kesehatanBadan->keperluanSurat ?? '') }}" :required="false" :readonly="$readonly">Keperluan Surat</x-text-input>
            </div>
